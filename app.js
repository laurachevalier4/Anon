...
hbs.registerHelper('drawChart', function(question_id) {
  var svg = d3.select(jsdoc.body).append("svg"); // individual svg has id #question_id
  ...
  var graph = d3.json("http://localhost:5000/api/" + question_id + "/voters.json", function(error, data) {
    ...
    let f = function(d) {
      ...
      console.log("d.answers", d.answers);
      return d;
    }
  ...
  // thought I could use promises but couldn't figure out how to return svg to the helper function instead of the fulfill function of the Promise's .then()
  // return svg;
});
